...
export function renderCurrent(state, now, weatherIcon) {
  ...
  const displayLocation = formatResolvedLocation(
    weather.location.label,
    currentPlace?.query,
  );
  const label =
    displayLocation.label ?? currentPlace?.label ?? "Selected location";
  const name =
    weather.location.city ??
    displayLocation.city ??
    currentPlace?.label ??
    label;
  const region = weather.location.region ?? displayLocation.region;
  const country = weather.location.country ?? displayLocation.country;
  const contextLabel = [region, country].filter(Boolean).join(" · ");
  ...
  return `<section class="weather-hero" aria-labelledby="location-title"><div class="hero-top"><div><p class="eyebrow">${esc(contextLabel || "CURRENT CONDITIONS")}</p><h1 id="location-title">${esc(name)}<span>.</span></h1><p class="resolved-location">${esc(label)}</p><p class="local-time" id="local-clock"></p></div><button class="favorite-button" data-action="favorite" data-focus="favorite" aria-label="${saved ? "Remove from" : "Add to"} saved locations" aria-pressed="${saved}">${saved ? "★" : "☆"}</button></div>
